...
private fun WelcomeStep() {
    ...
    Text("腺动", color = Teal, style = MaterialTheme.typography.headlineLarge)
    Spacer(Modifier.height(8.dp))
    Text("男性盆底肌训练", color = Ink, style = MaterialTheme.typography.headlineMedium)
...
private fun DisclaimerStep(agreed: Boolean, onAgree: (Boolean) -> Unit) {
    Text("使用前请了解", color = Ink, style = MaterialTheme.typography.headlineMedium)
    Spacer(Modifier.height(12.dp))
    DisclaimerBody()
    Spacer(Modifier.height(8.dp))
    androidx.compose.foundation.layout.Row(verticalAlignment = Alignment.CenterVertically) {
        Checkbox(checked = agreed, onCheckedChange = onAgree)
        Text("我已阅读并了解以上说明", color = Ink, style = MaterialTheme.typography.bodyMedium)
    }
}
...
private fun AgeStep(selected: AgeRange?, onChoose: (AgeRange) -> Unit) {
    Text("你的年龄段", color = Ink, style = MaterialTheme.typography.headlineMedium)
    Spacer(Modifier.height(8.dp))
    Text("用于安排起始强度，不会上传。", color = InkMuted, style = MaterialTheme.typography.bodyMedium)
    Spacer(Modifier.height(16.dp))
...
private fun ExperienceStep(selected: Experience?, onChoose: (Experience) -> Unit) {
    Text("盆底训练经验", color = Ink, style = MaterialTheme.typography.headlineMedium)
    Spacer(Modifier.height(16.dp))
    FlowRow(horizontalArrangement = Arrangement.spacedBy(10.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
...
private fun GoalsStep(selected: Set<Goal>, onToggle: (Goal) -> Unit) {
    Text("更想改善什么", color = Ink, style = MaterialTheme.typography.headlineMedium)
    Spacer(Modifier.height(8.dp))
    Text("可多选，今日主课会据此偏重。", color = InkMuted, style = MaterialTheme.typography.bodyMedium)
    Spacer(Modifier.height(16.dp))
    FlowRow(horizontalArrangement = Arrangement.spacedBy(10.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
...
private fun BudgetStep(selected: DailyBudget?, onChoose: (DailyBudget) -> Unit) {
    Text("每天能拿出多久", color = Ink, style = MaterialTheme.typography.headlineMedium)
    Spacer(Modifier.height(16.dp))
    FlowRow(horizontalArrangement = Arrangement.spacedBy(10.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
...
private fun GuideStep(page: Int) {
    ...
    Text("找对盆底肌 ${page + 1}/3", color = Teal, style = MaterialTheme.typography.titleMedium)
    Spacer(Modifier.height(12.dp))
    Text(title, color = Ink, style = MaterialTheme.typography.headlineMedium)
    Spacer(Modifier.height(12.dp))
    Text(body, color = InkMuted, style = MaterialTheme.typography.bodyLarge)
}

@Composable
private fun BaselineStep(ui: OnboardUi) {
    Text("可选：最长保持测试", color = Ink, style = MaterialTheme.typography.headlineMedium)
    Spacer(Modifier.height(8.dp))
    Text(
    ...
    Text(
        ...
        color = if (ui.baselineRunning) Amber else Ink,
        style = MaterialTheme.typography.displayLarge,
    )
    if (ui.baselineRunning) {
        Spacer(Modifier.height(8.dp))
        Text("保持收缩，不要憋气", color = InkMuted)
    }
}

@Composable
private fun PreviewStep(ui: OnboardUi) {
    Text("你的 28 天计划已就绪", color = Ink, style = MaterialTheme.typography.headlineMedium)
    Spacer(Modifier.height(8.dp))
    Text("今日主课按此表推送。课程库随时可以加练。", color = InkMuted, style = MaterialTheme.typography.bodyMedium)
    Spacer(Modifier.height(16.dp))
    ui.preview.chunked(7).forEachIndexed { week, days ->
        AppCard(modifier = Modifier.padding(bottom = 10.dp)) {
            Text("第 ${week + 1} 周", color = Teal, style = MaterialTheme.typography.titleMedium)
            Spacer(Modifier.height(8.dp))
            days.forEach { day ->
